-- +1 MVP Supabase schema and security repair
...
CREATE OR REPLACE FUNCTION public.submit_application(
  ...
  p_quantity INTEGER,
  p_expiry_date DATE,
  p_unit_base_price INTEGER,
  p_initial_ratio INTEGER,
  p_initial_price INTEGER,
  p_had_price_offer BOOLEAN,
  p_offered_ratio INTEGER DEFAULT NULL,
  p_offered_price INTEGER DEFAULT NULL,
  p_offer_accepted BOOLEAN DEFAULT NULL,
  p_final_ratio INTEGER DEFAULT NULL,
...
BEGIN
  ...
  END IF;
  IF p_unit_base_price IS NULL OR p_unit_base_price < 0 THEN
    RAISE EXCEPTION '기준가격을 확인해주세요.';
  END IF;
  IF p_initial_ratio IS NULL OR p_initial_ratio NOT BETWEEN 0 AND 100
  ...
  END IF;
  IF p_had_price_offer
    AND (p_offered_ratio IS NULL OR p_offered_price IS NULL OR p_offer_accepted IS NULL) THEN
