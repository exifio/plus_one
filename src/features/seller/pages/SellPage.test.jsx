/*
 * 관련 작업: FE-6·FE-7 — 판매자 5단계 신청 화면과 모집 상태 가드.
 * 작성 이유: 필수 상품 정보·증빙·연락처를 순서대로 받은 뒤에만 신청할 수 있어야 하기 때문.
 * 확인 내용: 단계 이동, 필수값 비활성화, 증빙 업로드, 확인 화면, 제출 완료, paused/closed/error 차단.
 */
...
async function moveToEvidence(user) {
  await openSellPage();
  ...
  await user.click(screen.getByRole('button', { name: '보관상품 확인하기' }));
}

async function moveToReview(user) {
  await moveToEvidence(user);
  ...
  await user.click(screen.getByRole('button', { name: '신청 내용 확인하기' }));
}
...
describe('판매자 5단계 신청', () => {
  ...
  test('유효기간 없이도 상품 정보가 유효하면 보관상품 확인 단계로 이동한다', async () => {
    ...
    expect(screen.getByRole('heading', { name: '보관 중인 상품을 확인할게요' })).toBeInTheDocument();
  });

  test('증빙 이미지가 있어야 연락처 단계로 이동한다', async () => {
    const user = userEvent.setup();
    await moveToEvidence(user);
  ...
  test('연락처가 유효하면 신청 확인 단계로 이동한다', async () => {
    const user = userEvent.setup();
    await moveToReview(user);

    expect(screen.getByText('5 / 5')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '판매 신청 내용을 확인해주세요' })).toBeInTheDocument();
    expect(screen.getByAltText('보관상품 확인 이미지')).toBeInTheDocument();
